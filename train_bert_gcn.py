...
parser.add_argument('--dataset', default='20ng', choices=['20ng', 'R8', 'R52', 'ohsumed', 'mr','ms'])
...
########################
print(f"Size of train_idx dataset: {len(train_idx)}")
print(f"Size of val_idx dataset: {len(val_idx)}")
print(f"Size of test_idx dataset: {len(test_idx)}")
print(f"Size of doc_idx dataset: {len(doc_idx)}")
#####################
...
metrics = {
    ...
    'f1': MetricsLambda(
        lambda x, y: 2 * (x * y / (x + y + 1e-20)).mean(),
...
def log_training_results(trainer):
    ...
    for batch in idx_loader_test:
        batch_y_pred, batch_y_true = test_step(evaluator, batch)
        y_true.extend(batch_y_true.cpu().numpy())
        y_pred.extend(batch_y_pred.argmax(axis=1).cpu().numpy())
    
    test_f1 = f1_score(y_true, y_pred, average='weighted')
     
    
    logger.info(
        "Epoch: {}  Train acc: {:.4f} loss: {:.4f}  Val acc: {:.4f} loss: {:.4f}  Test acc: {:.4f} loss: {:.4f} Test F1-Score: {:.4f}"
        .format(trainer.state.epoch, train_acc, train_nll, val_acc, val_nll, test_acc, test_nll,test_f1)
    )
